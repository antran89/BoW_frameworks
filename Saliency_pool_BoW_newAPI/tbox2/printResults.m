...
% saliency params
fprintf(fid, 'number of pooling regions in a video: %d\n', params.num_regions);
fprintf(fid, 'saliency weights on different regions:');
for r = 1:length(params.weights)
    fprintf(fid, ' %.4f', params.weights(r));
end
fprintf(fid, '\n');
...
for idx = 1:l
    ...
    for i = 1:params.no_splits
        fprintf(fid, '%.2f ', perclass_acc(idx, i));
    end
    fprintf(fid, '\n');
end
