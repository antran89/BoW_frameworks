...
%% set dataset params
params.dataset = 'SSBD';
params.afspath = '~/Public/data/video/ssbd';
...
% path to features
params.featurepath = '~/Public/data/video/features/ImprovedDenseTrajectories/SSBD';
...
params.encoding = 'Fisher';             % {VQ, SA, SA-k, LLC, myLLC, SPC, Fisher, FK, VLAD, SVC}, Fisher using Yael lib, FK using Encoding toolbox
params.pooling = 'sum-pooling';         % {sum-pooling, max-pooling}
params.normalization = 'Power+L2';      % {L1, L2, Power, Power+L1, Power+L2, Intra+Power+L2}
...
params.regionindex = '~/Public/data/video/features/ImprovedDenseTrajectories/SSBD';
% number of levels in saliency pyramid
params.num_regions = 4; % indicate number of spatial-temporal regions in each video, which replace num_levels in future format
...
params.no_splits = 5;           % 5-fold validation cross setup on SSBD, 25 videos/class => 5 splits
