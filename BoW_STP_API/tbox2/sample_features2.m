...
function [Xall] = sample_features2(params, data_file_names, label_file_names, threshold)
%SAMPLE_FEATURES Summary of this function goes here
%   Detailed explanation goes here
%   Sampling features to train a codebook
%   This is different from sample_features in the sense. It only samples
%   from a number of movies. So, the sampling process will be faster.
%   input: params
...
if isfield(params, 'num_movies') && params.num_movies > 0 && params.num_movies < m
    p = randperm(m);
    p = p(1:params.num_movies);     % if no input, take all movies
    data_file_names = data_file_names(p,:);
    label_file_names = label_file_names(p);
end
...
m = length(data_file_names);    % if we subsample data_file_names
Xall_fill = 0;
p = ceil(threshold/m);
