...
addpath(fullfile(path, '/tbox2/'));         % specific tbox for BoW
...
%% set dataset params
params.dataset = 'HMDB';
params.afspath = '~/Public/data/video/HMDB';
...
params.avipath = fullfile(params.afspath, 'Video/'); 
params.infopath = fullfile(params.afspath, 'TrainTestSplit/TrainTestSplit/'); %path for video labels/info

% path to features
params.featurepath = '~/Public/data/video/features/iDT/HMDB2';
...
if strcmp(params.encoding, 'Fisher')
    ...
    params.num_centroids = 512;         % gmm mixtures 
else
...
assert(l == 51, 'BoW_main :: something wrong with reading classes files');
...
params.no_splits = 3;           %number of splits for HMDB51 dataset: 3 splits
[all_train_labels, all_test_labels, ~, ~] = get_data_summary(params, 1);
params.no_train_samples = length(all_train_labels);
params.no_test_samples = length(all_test_labels);
